...
- `main.py`: Complete pipeline script that loads data, performs EDA, trains multiple models, and saves the top one.
- `requirements.txt`: List of Python dependencies required to run the project.
- `final_churn_model.pkl`: The best-performing trained machine learning model (saved automatically).
- `cat_options.pkl`: Dictionary of unique values for categorical features (used for menus in the app).
...
6. Open the link displayed in the terminal (usually `http://localhost:8501`) in your web browser.
...
1. **Data Pipeline:** `main.py` imports the Excel dataset and cleans column names.
2. **Preprocessing:**
...
- **Missing Value Analysis:** Inspects and reports null values.
- **Correlation Mapping:** Produces `eda_correlation_matrix.png` to show relationships between numerical features.
...
- XGBoost Classifier (Often picked as the best performer)
