...
- `app.py`: Streamlit web application that accepts user input and displays churn probability.
- `main.py`: Complete pipeline script that loads data, conducts EDA, trains multiple models, and saves the peak one.
...
4. **Train the model (Starting Run Only):**
   *Note: You must trigger this script first to generate the model files.*
...
- **Prediction:** Use the Streamlit web interface to tweak customer parameters (like Tenure, Satisfaction Score, etc.) and click "Predict Churn Risk" to see if the customer is Safe or at High Risk.

## How It Works

1. **Data Pipeline:** `main.py` reads the Excel dataset and cleans column names.
2. **Preprocessing:**
...
4. **Deployment:** `app.py` loads this saved pipeline and uses it to execute inference on live user input.
...
- **Class Balance:** Examines the percentage of Churn vs. Non-Churn customers.
...
- XGBoost Classifier (Often designated as the best performer)
